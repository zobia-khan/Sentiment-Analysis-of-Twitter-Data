...
def pre_process_urdu(filename):
        ...
        visualizations_urdu(scrapped_file_path)
        feature_extraction_urdu(scrapped_file_path)
        report_urdu(scrapped_file_path,unique_filename,'Urdu')
...
def visualizations_urdu(filename):
   ...
   # Convert the Pandas Series to a single string
    ...
    print(text)
    # Word Cloud
    wc = WordCloud(font_path='Faiz Lahori Nastaleeq.ttf', max_words=1000, width=1600, height=800, collocations=False).generate(text)
...
def report_urdu(filename,uniqueFilename,language):
    ...
    class PDF(FPDF):
        ...
        def header(self):
            ...
            self.add_font('nafees', '', 'Nafees Regular.ttf', uni=True)

            self.set_font('nafees','', 12)
    ...
    pdf.add_page()
    # Set the Urdu font
    pdf.add_font('nafees', '', 'Nafees Regular.ttf', uni=True)
    ...
    pdf.set_font('nafees', '', 12)
    for i in range(len(absText)):
        pdf.text(110, 170 + i*5, absText[i])

    pdf.add_page()
    # Add PERFORMANCE SUMMARY to the PDF
    pdf.set_font('Arial', 'B', 16)
    pdf.cell(0, 10, 'DATA ANALYTICS REPORT', 0, 1, 'C')
    pdf.set_font('Arial', '', 12)
    pdf.cell(0, 10, 'DATA SUMMARY:', 0, 1, 'L')
    pdf.cell(0, 10, f'Language: {language}', 0, 1, 'L')
    pdf.cell(0, 10, 'ANALYSIS BY: Zobia', 0, 1, 'L') # add user name 
    pdf.cell(0, 10, f'NUMBER OF TWEETS: {len(text_column)}', 0, 1, 'L')
    ...
    pdf.set_font('Arial', '', 12)
    visualization_text = '''Word cloud visually represents word importance and prevalence, with larger fonts for higher frequency.
The frequency analysis displays the top 30 most common words and their counts, providing deeper insights into your data.'''
    ...
    pdf.add_page()
    pdf.set_font('Arial', 'B', 16)
    pdf.cell(0, 10, 'SENTIMENT PERCENTAGE ON GIVEN DATA', 0, 1, 'C')
    pdf.set_font('Arial', '', 12)
    sentiment_text = '''Here you can view how the sentiments are of individuals with respect to percentage.
    The x-axis contains the sentiments, the representation being:
    o 0: negative
    o 1: positive
    And y-axis being the number of tweets from the dataset before bar plot image'''
    #pdf.cell(40, 10, 'Word Cloud', 0, 1)
...
# visualizations_urdu('media/clean/2596be08-0870-4f21-b254-dc5fb7529b70_clean.csv')       
# #pre_process_urdu('media/scrapped/Pure_urdu_data/urdu_pure2.csv')
