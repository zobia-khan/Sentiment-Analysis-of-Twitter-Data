...
def pre_process(filename):
    ...
    report(scrapped_file_path,unique_filename,'English')
...
def feature_extraction_english(filename):
    ...
    # Loading model
    lr_english_model = pickle.load(open('media/models/english/LRmodel_sklearn.sav','rb')) #give path of best model
...
def report(filename,uniqueFilename,language):
    # Custom class to overwrite the header and footer methods
    class PDF(FPDF):
        def __init__(self):
            super().__init__()
...
The frequency analysis displays the top 30 most common words and their counts, providing deeper insights into your data.'''
    ...
    pdf.multi_cell(0, 10, sentiment_text)
...
#pre_process('media/scrapped/JohnnyDepp_BPTSPADOPF.csv')    
